        ...
        <header class="px-4 sm:p-5 shadow bg-white">
            @auth  
                ...
                <div class="flex md:hidden justify-between items-center">
                    <a class="text-xl md:text-3xl font-black" href="{{ route('home')}}">Devstagram</a>
            ...
            <div class="container mx-auto hidden md:flex justify-between items-center">
                ...
                @auth
                    <nav class="flex gap-2 items-center">
                        ...
                        <a href="{{ route('buscador.index') }}" class="flex items-center gap-2 bg-white border p-2 text-gray-500 rounded text-sm cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                            </svg>
                            Buscador
                        </a>
                        <a class="flex items-center gap-2 bg-white border p-2 text-gray-500 rounded text-sm cursor-pointer" href="{{ route('posts.create')}}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6.827 6.175A2.31 2.31 0 0 1 5.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 0 0-1.134-.175 2.31 2.31 0 0 1-1.64-1.055l-.822-1.316a2.192 2.192 0 0 0-1.736-1.039 48.774 48.774 0 0 0-5.232 0 2.192 2.192 0 0 0-1.736 1.039l-.821 1.316Z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 12.75a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0ZM18.75 10.5h.008v.008h-.008V10.5Z" />
                            </svg>
                        ...
                        <a class="text-gray-500 text-sm flex items-center gap-1" href="{{ route('posts.index', auth()->user()->username)}}">
                            <img class="rounded-full w-13" src="{{ auth()->user()->imagen ? asset('perfiles').'/'. auth()->user()->imagen : asset('img/usuario.svg')}}" alt="Imagen usuario"> 
                            <span class="">{{ auth()->user()->username }}</span>
                        </a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="font-bold uppercase text-gray-600 text-sm cursor-pointer flex text-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
